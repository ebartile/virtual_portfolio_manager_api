...
const newCollabJobForBusiness = async(req,res)=>{
    try{
        ...
        let { business_id,business_owner_username,collab_title,collab_amount,collab_description } = req.body;
        ...
        const newCollab = await CollabModel.create({
            ...
            businessOwnerUsername   :   business_owner_username,
        ...
        return res.status(500).json({
            result:true,
            msg:"New Collab job added to the business."
        })
    }
    catch(err){
        console.log(err);
...
const getAllCollabJobForABusiness = async(req,res)=>{
    try{
        ...
        if(businessID == null || businessID == undefined){
            return res.status(400).json({
                result:false,
                msg:"Business ID is not valid"
            })
        }

        const collabJobs = await CollabModel.find({businessID,collaboratorSearchActive:true});

        if(collabJobs.length == 0){
            return res.status(404).json({
                result:false,
                msg:"No collab jobs for business found"
            })
        }

        return res.status(200).json({
            result:true,
            msg:"Collab jobs for the businesses fetched",
            collabJobs
        })
        ...
        res.status(500).json({
            result:false,
            msg:"There was a problem fetching collab jobs for a business"
        });
    }
}
...
const getAllCollabRequestForABusinessJob = async(req,res)=>{
    try{
        ...
        if(collab[0].collabRequests.length == 0){
            return res.status(404).json({
                result:false,
                msg:"No collab request found for the business."
            })
        }
        ...
        res.status(500).json({
            result:false,
            msg:"There was a problem fetching collab request for a jobs in a business"
        });
    }
}
...
const acceptUsersRequest = async(req,res)=>{
    try{
        ...
        // Saving collab data in the business
        ...
        const txObject = {
            ...
            data: contract.methods.collab(col_address,business_address,collab_title,collab_amount).encodeABI()
        }
        ...
        // Updating the col's businesses_collaborated list

        const userDetails = await UserDetailsModel.findOneAndUpdate({username:col_username},{
            $addToSet : {
                businesses_collaborated : collab_job_id
            }
        });

        if(!userDetails){
            return res.status(500).json({
                msg:"There was a problem updating the businesses_collaborated list of the col",
                result:false
            })
        }
...
const rejectCollabRequestForABusinessJob = async(req,res)=>{
    try{
        //Input field validation
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
...
const getCollabAcceptedRequest  =   async(req,res) =>{
    try{
        ...
        let businessAddress = req.params.businessaddress;

        if(businessAddress == null || businessAddress.length == 0){
            return res.status(404).json({
                msg:"Business address is not valid",
                result:false
            })
        }

        let collabJobs = await contract.methods.getCollabDetails(businessAddress).call();
...
module.exports = {
    newCollabJobForBusiness,
    getAllCollabJobForABusiness,
    sendRequestToCollab,
    sendRequestToCollab,
    getAllCollabRequestForABusinessJob,
    acceptUsersRequest,
    rejectCollabRequestForABusinessJob,
